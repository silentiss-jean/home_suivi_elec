...
import { renderDuplicates } from "./duplicatesPanel.js";

console.info("[config.view] module chargé (pliage fiable, délégation doublons)]");

// ✅ renderReferencePanel SUPPRIMÉ - géré par referencePanel.js

export function ensureUserConfigAbove(contentEl) {
  const cfgRoot = contentEl?.parentElement;
  const userCard = document.getElementById("userConfig");
  if (!contentEl || !cfgRoot || !userCard) return;
  if (userCard.nextSibling !== contentEl) {
    cfgRoot.insertBefore(userCard, contentEl);
  }
}

function makeToggleHeader(integrationKey, columnKey, titleHTML, panelEl) {
...
export function renderColumns(container, ctx) {
  const { selected, alternatives, refEntityId, handlers } = ctx || {};
  if (!container) return;
