...
# ---------------------------------------------------------------------------
# SETUP PANEL (HTML DIRECT via panel_custom, AVEC IFRAME pour callWS)
# ---------------------------------------------------------------------------
...
async def async_setup_panel(hass: HomeAssistant):
    ...
    # Enregistre le panneau via panel_custom pour que callWS fonctionne
    if not hass.data.get("home_suivi_elec_panel_registered"):
        ...
        _LOGGER.info("[PANEL] ✅ Panneau HTML Home Suivi Élec (iframe) ajouté à la barre latérale.")
    else:
